...
def test_train_set_split(Data,train_columns,target_columns,split_year,Normalization_log,Normalization_test,Normalization_train):
  ...
  return Data_test,Data_sans_anomalie_train, X_Train, Y_train, X_test, Y_test
...
def plot_results(history,model,Data_train,Data_test,X_train,Y_train,vertical_profiles_depth,Normalisation,target_columns):
  ...
  if Normalisation ==True:
    ...
    fig=plt.figure(figsize=(64,64))
    y_pred_train=model.predict(X_train)
    y_pred_train=Eliminer_annee_moyenne(y_pred_train,-year_average(Data_train[Data_train['year']>=split_year][target_columns.append('5days')]))
    Y_train=Eliminer_annee_moyenne(Y_train,-year_average(Data_train[Data_train['year']>=split_year][target_columns.append('5days')]))
    ax1=plt.subplot(511)
    im=ax1.imshow(Y_train.T,cmap=plt.cm.jet)
    ax1.set_title('Profil des vraies valeurs',size=20)
    plt.colorbar(im,fraction=0.013, pad=0.01)
