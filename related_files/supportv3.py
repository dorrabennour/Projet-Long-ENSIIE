...
def test_train_set_split(Data,train_columns,target_columns,split_year,Normalization_log,Normalization_test,Normalization_train):
  ...
  lookback_window = 73   # 73 weeks = 1 year per localisation
...
def plot_results(history,model,Data,days_train,X_train,Y_train,vertical_profiles_depth,Normalisation,target_columns):
  ...
  if Normalisation ==True:
    ...
    im=ax1.imshow(Y_train.T,cmap=cmc.cm.balance) #cmap=plt.cm.jet
    # ensemble_affichage=[i for i in range(len(Y_train.shape[0]))  if i%73==0]
    ...
    im=ax2.imshow(y_pred_train.T,cmap=plt.cm.RdBu)
    ...
    ax1=plt.subplot(511)
    im=ax1.imshow(Y_train.T,cmap=plt.cm.RdBu)
    ax1.set_title('Profil des vraies valeurs',size=20)
    plt.colorbar(im,fraction=0.013, pad=0.01)
    ensemble_affichage_y=[i for i in range(18) ]
    plt.yticks(ensemble_affichage_y,["%.2f" % elem for elem in vertical_profiles_depth],size=10)
...
def TCN_model(X_Train,Y_train,batch_size,activation,dilations,nbfilters,kernelsize, nbstacks,
  ...
  batch_size, time_steps, input_dim = batch_size, 73, X_Train.shape[2]
...
def fit_TCN_model(model,expanding_window_splitter,btscv_splitter,X_Train,Y_train,batch_size,nb_epochs):
  ...
  if (expanding_window_splitter == True and btscv_splitter == False):
    btscv = expanding_window(horizon = 73,period = 73)
...
def RMSE_Tranche_1(x_true, x_pred):
  from keras import backend as K
  return K.sqrt(K.mean(K.square(x_pred[:,:2] - x_true[:,:2])))
def RMSE_Tranche_2(x_true, x_pred):
  from keras import backend as K
  return K.sqrt(K.mean(K.square(x_pred[:,2:4] - x_true[:,2:4])))
def RMSE_Tranche_3(x_true, x_pred):
  from keras import backend as K
  return K.sqrt(K.mean(K.square(x_pred[:,4:6] - x_true[:,4:6])))
def RMSE_Tranche_4(x_true, x_pred):
  from keras import backend as K
  return K.sqrt(K.mean(K.square(x_pred[:,6:8] - x_true[:,6:8])))
def RMSE_Tranche_5(x_true, x_pred):
  from keras import backend as K
  return K.sqrt(K.mean(K.square(x_pred[:,8:10] - x_true[:,8:10])))
def RMSE_Tranche_6(x_true, x_pred):
  from keras import backend as K
  return K.sqrt(K.mean(K.square(x_pred[:,10:12] - x_true[:,10:12])))
# dx = (Y_train).argsort()[:int(len(Y_train)*0.1)]
# np.min(Y_train)
# Y_train[dx,:]
def RMSE_Tranche_7(x_true, x_pred):
  from keras import backend as K
  return K.sqrt(K.mean(K.square(x_pred[:,12:14] - x_true[:,12:14])))
def RMSE_Tranche_8(x_true, x_pred):
  from keras import backend as K
  return K.sqrt(K.mean(K.square(x_pred[:,14:16] - x_true[:,14:16])))
def RMSE_Tranche_9(x_true, x_pred):
  from keras import backend as K
  return K.sqrt(K.mean(K.square(x_pred[:,16:19] - x_true[:,16:19])))
